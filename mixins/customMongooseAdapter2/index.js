...
class MongooseDbAdapter {
	 ...
	 */
	connect() {
		for (let key of Object.keys(this.listAdapter)){
            let value = this.listAdapter[key];
            let conn;
            if (this.model) {
                /* istanbul ignore next */
                if (mongoose.connection.readyState == 1) {
                    ...
                    conn = mongoose.connect(value.uri, value.opts);
                }
            } else if (this.schema) {
                conn = mongoose.createConnection(value.uri, value.opts);
                this.model = conn.model(value.modelName, value.schema);
            }
