	...
	events: {
		'CW4973-media.get-media-link': {
			async handler(ctx: Context<any>) {
				...
				// @ts-ignore
				this.logger.debug('get-media-link ctx.params', uri, media_link_key, CONTRACT_TYPE.CW4973);
...
export default class CrawlAssetService extends moleculer.Service {
	...
	async getMediaLink(uri: string, file_name: string, key: string) {
		...
		const media: any[] = await this.broker.call(CW4973_MEDIA_MANAGER_ACTION.FIND, { query }, OPTs);
		...
		if (media.length === 0) {
			...
			await this.broker.call(CW4973_MEDIA_MANAGER_ACTION.UPDATE_MEDIA_LINK, { uri, file_name, key }, OPTs);
		} else {
