...
export default class CrawlAssetService extends moleculer.Service {
	...
	async getMediaLink(uri: string, file_name: string, key: string, chain_id: string) {
		...
		if (media.length === 0) {
			...
			await this.broker.call(CW721_MEDIA_MANAGER_ACTION.UPDATE_MEDIA_LINK, { uri, file_name, key, chainId: chain_id }, OPTs);
		} else {
	...
	async _start(): Promise<void> {
		// let uri = 'https://filesamples.com/samples/image/svg/sample_1920%C3%971280.svg';
		// let key = Common.getKeyFromUri(uri);
		// await Common.handleUri(uri, 'a3');
