...
export default class CrawlAssetService extends moleculer.Service {
	...
	async handleJob(ctx: Context, code_id: Number) {
		...
		const getInforPromises = await Promise.all(contractList.map(async (address) => {
	...
	async checkIfContractImplementCW721Interface(code_id: Number) {
		...
		do {
			...
			if (resultCallApi?.contracts?.length > 0) {
				...
				while (i < resultCallApi.contracts.length) {
					...
					let listTokenIDs = await this.callApi(urlGetListToken);
					if (listTokenIDs?.data?.tokens !== undefined) {
						if (listTokenIDs.data.tokens.length > 0) {
							...
							let tokenInfo = await this.callApi(urlGetOwner);
		...
		this.logger.debug("Check if cw721 interface implemented", cw721flag == null ? Status.TBD : cw721flag);
		switch (cw721flag) {
			case null: {
				this.broker.call("code_id.updateMany", { condition: { code_id: code_id }, update: { status: Status.TBD } });
				break;
			}
			case true: {
				this.broker.call("code_id.updateMany", { condition: { code_id: code_id }, update: { status: Status.COMPLETED } });
				this.broker.emit("code_id.handle", code_id);
				break;
			}
			case false: {
				this.broker.call("code_id.updateMany", { condition: { code_id: code_id }, update: { status: Status.REJECTED } });
				break;
			}
		}
	}
	...
	private async getTokenList(ctx: Context<TokenInfo, RetryTime>) {
		...
		try {
			...
			if (listTokenIDs?.data?.tokens !== undefined && listTokenIDs.data.tokens.length > 0) {
				...
				const getInforPromises = await Promise.all(listTokenIDs.data.tokens.map(async (token_id: String) => {
					...
					await ctx.call(ACTION_GET_TOKEN_INFOR, { code_id, address, token_id }, { timeout: ACTION_TIMEOUT, retries: MAX_RETRY_REQ });
				}));
				await getInforPromises;
	...
	private async getTokenInfor(ctx: Context<TokenInfo, RetryTime>) {
		...
		try {
			...
			if (tokenInfo?.data?.access?.owner !== undefined) {
				const asset = await this.createAssetObject(ctx.params.code_id, ctx.params.address, ctx.params.token_id, tokenInfo);
