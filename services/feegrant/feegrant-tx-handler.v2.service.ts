...
export default class FeegrantTxHandler extends Service {
	private _currentId: Types.ObjectId | string | null = null;
	...
	async initEnv() {
		// Get feegrant latest id which was unprocessing
		const handledIdRedis: string = await this.redisClient.get(
			Config.REDIS_KEY_CURRENT_FEEGRANT_ID,
		);
		this._currentId = null;
		// Oldest id in Tx DB
		const oldestIdTx: ITransaction[] = await this.adapter.lean({
			sort: '_id',
			limit: 1,
			projection: {
				_id: 1,
		...
		this._currentId = handledIdRedis ? Types.ObjectId(handledIdRedis) : oldestIdTx[0]._id;
	}

	async handleJob(chainId: string): Promise<any[]> {
		const feegrantList: IFeegrantData[] = [];
		// Get all transactions sequence by id, limit 2000, start from _currentId
		...
		if (listTx.length > 0) {
			this.logger.info(
				`Feegrant from  ${this._currentId} to ${listTx[listTx.length - 1]._id}`,
			);
			...
			// Update feegrant latest id
			this._currentId = listTx[listTx.length - 1]._id;
			this.redisClient.set(Config.REDIS_KEY_CURRENT_FEEGRANT_ID, this._currentId?.toString());
