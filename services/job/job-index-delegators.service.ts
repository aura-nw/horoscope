...
export default class IndexDelegatorsService extends Service {
	...
	async handleJob(lastId: number) {
		...
		for (let addr of listAddresses) {
			queryMySQL.push(
				knex('delegator_rewards')
					.select(knex.raw(`*, max(id) as last_id, sum(amount) as total_rewards`))
					.where({ delegator_address: addr.delegator_address })
					.groupBy('validator_address')
					.orderBy('last_id'),
			);
		...
		if (resultMySQL.length > 0) {
			resultMySQL.map((res: any) => {
				let account = resultMongo.find(
					(a: any) =>
						a.address === res[0].delegator_address &&
