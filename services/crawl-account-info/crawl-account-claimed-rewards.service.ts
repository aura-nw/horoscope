				...
				try {
					switch (msg['@type']) {
						case MSG_TYPE.MSG_DELEGATE:
							...
							const indexReward = tx.tx_response.logs[index].events
								.find((x: any) => x.type === CONST_CHAR.COIN_RECEIVED)
								.attributes.findIndex((x: any) => x.value === userAddress);
							const claimedReward = tx.tx_response.logs[index].events.find(
						...
						case MSG_TYPE.MSG_REDELEGATE:
							...
							const coinReceived = tx.tx_response.logs[index].events.find(
								(x: any) => x.type === CONST_CHAR.COIN_RECEIVED,
							).attributes;
						...
						case MSG_TYPE.MSG_UNDELEGATE:
							...
							const undelegateIndexReward = msg.events
								.find((x: any) => x.type === CONST_CHAR.COIN_RECEIVED)
								.attributes.findIndex((x: any) => x.value === userAddress);
							const undelegateClaimedReward = tx.tx_response.logs[index].events.find(
								(x: any) => x.type === CONST_CHAR.COIN_RECEIVED,
						...
						case MSG_TYPE.MSG_WITHDRAW_REWARDS:
							const log = tx.tx_response.logs[index];
