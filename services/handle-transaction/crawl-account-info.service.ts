...
export default class CrawlAccountInfoService extends Service {
    ...
    public constructor(public broker: ServiceBroker) {
        ...
        this.parseServiceSchema({
            ...
            queues: {
                'crawl.account-info': {
                    ...
                    async process(job: Job) {
                        job.progress(10);
                        // @ts-ignore
                        await this.handleJob(job.data.address, job.data.message);
                        job.progress(100);
                        return true;
                    },
                }
            ...
            events: {
                'account-info.upsert': {
                    handler: (ctx: any) => {
                        this.logger.debug(`Crawl account info by address: ${ctx.params.address}`);
                        this.createJob(
                            'crawl.account-info',
                            {
    ...
    async handleJob(address: String, message: String) {
        ...
        if (accountInfo._id) {
            try {
                let res = await this.adapter.updateById(accountInfo._id, accountInfo);
            ...
            try {
                let res = await this.adapter.insert(accountInfo);
                this.logger.debug(res);
            } catch (error) {
                this.logger.error(error);
            }
