...
import {
	...
	txExecuteContractCreateMinter,
	txExecuteContractMint,
	instantiatedContract,
	createMinterExecuteContractOne,
	createMinterExecuteContractTwo,
	mintContract,
	txInstantiateCw721Contract,
	instantiatedCw721Contract,
} from './mock-data';
...
describe('Test contract-tx-handler service', () => {
	...
	it('Should crawl new instantiated cw721 contract', async () => {
		await smartContractService._handleJob([txInstantiateCw721Contract], Config.CHAIN_ID);

		const result = await smartContractService.adapter.findOne({
			contract_address: 'aura1t7sv20kw5vm8gkpzrak4qfmxxsktdc9ykdjay5kr5lr8frtskwwqdnd6re',
		});

		expect(_.omit(result.toObject(), ['_id', '__v'])).toEqual(instantiatedCw721Contract);
	});
	...
	it('Should crawl new create_minter execute contract', async () => {
		...
		expect(_.omit(resultOne.toObject(), ['_id', '__v'])).toEqual(
			createMinterExecuteContractOne,
		);
		expect(_.omit(resultTwo.toObject(), ['_id', '__v'])).toEqual(
			createMinterExecuteContractTwo,
		);
	});
